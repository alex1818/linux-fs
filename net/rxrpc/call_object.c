...
found_user_ID_now_present:
	write_unlock(&rx->call_lock);
	set_bit(RXRPC_CALL_RELEASED, &call->flags);
	__rxrpc_set_call_completion(call, RXRPC_CALL_LOCAL_ERROR, 0, -EEXIST);
	rxrpc_put_call(call);
...
void rxrpc_see_call(struct rxrpc_call *call)
{
	...
	if (call) {
		...
		trace_rxrpc_call(call, 2, n, here, 0);
	}
}
...
void rxrpc_get_call(struct rxrpc_call *call)
{
	...
	trace_rxrpc_call(call, 3, n, here, 0);
}

/*
 * Note the addition of a ref on a call.
 */
struct rxrpc_call *rxrpc_get_call_maybe(struct rxrpc_call *call)
{
	const void *here = __builtin_return_address(0);
	bool ret = atomic_inc_not_zero(&call->usage);
	int n = atomic_read(&call->usage);

	if (ret)
		trace_rxrpc_call(call, 3, n, here, 0);
	return ret ? call : NULL;
}
...
void rxrpc_release_call(struct rxrpc_sock *rx, struct rxrpc_call *call)
{
	...
	call->recvmsg_link.next = NULL; /* list_empty() must return false */
...
void rxrpc_put_call(struct rxrpc_call *call)
{
	...
	_enter("%p{%d,u=%d}", call, call->debug_id, atomic_read(&call->usage));
