				    ...
				    long *timeo)
{
	...
	_enter(",{%u,%u,%u}",
	       call->tx_cursor, call->tx_top, call->tx_winsize);
	...
	for (;;) {
		...
		ret = 0;
		if (call->tx_top - call->tx_cursor < call->tx_winsize)
			break;
			   ...
			   struct msghdr *msg, size_t len)
{
	...
	do {
		...
		if (sp->remain <= 0 ||
			...
			else if (call->tx_top - call->tx_cursor <
				 call->tx_winsize)
